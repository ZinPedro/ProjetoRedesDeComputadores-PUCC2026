...
THREAD_FUNC(enviar_mensagens)
{
    ...
    while (verificar_conectado(cliente))
    {
        ...
        int pronto = entrada_disponivel(200);
        ...
        if (pronto < 0)
        {
            mostrar_erro_socket("Erro ao verificar entrada do teclado");
            definir_conectado(cliente, 0);
            break;
        }
    ...
    if (!cliente->pediu_quit)
    {
        printf("Conexao com o servidor perdida. Encerrando...\n");
        fflush(stdout);
    }
